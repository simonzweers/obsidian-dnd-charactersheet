<script lang="ts">
	...
	$: spellLevelKeys = Object.keys(char_spells)
		...
		.sort(
			(a, b) =>
				parseInt(a.replace("lvl", "")) - parseInt(b.replace("lvl", "")),
		);

	let newCantripName = "";
	let newSpellNames: Record<string, string> = {}; // keyed by levelKey

	async function updatePrimitiveField<
		K extends keyof DndCharacterFrontmatter,
	>(
		...
		await app.fileManager.processFrontMatter(
			file,
			(frontmatter: DndCharacterFrontmatter) => {
	...
	async function toggleDeathSave(
		...
		const updated = char_deathsaves[type].map((v, i) =>
			i === index ? !v : v,
		);
	...
	async function updateHitDice(field: keyof HitDice, value: string | number) {
		...
		await app.fileManager.processFrontMatter(
			file,
			(frontmatter: DndCharacterFrontmatter) => {
				if (!frontmatter.hit_dice)
					frontmatter.hit_dice = { total: 1, used: 0, die: "d8" };
	...
	function getAttackBonus(attack: Attack) {
		...
		const abilityKeysToCheck = isFinesse
			? ["str", "dex"]
			: [attack.ability?.toLowerCase()];
		const best = Math.max(
			...abilityKeysToCheck.map((a) =>
				char_abilities[a] !== undefined
					? getAbilityModifier(char_abilities[a])
					: -Infinity,
			),
		);
	...
	async function addAttack() {
		...
		const attackToAdd: Attack = {
			...newAttack,
			properties: newAttackProperties,
		};
	...
	async function toggleSpellPrepared(levelKey: string, index: number) {
		...
		char_spells = {
			...char_spells,
			[levelKey]: { ...level, learned: updatedLearned },
		};

		await app.fileManager.processFrontMatter(file, (fm) => {
			if (!fm.spells?.[levelKey]?.learned) return;
			fm.spells[levelKey].learned[index].prepared =
				updatedLearned[index].prepared;
		});
	}
	...
	async function pasteSpellLink(levelKey: string, index: number) {
		...
		if (levelKey === "cantrips") {
			const updated = char_spells.cantrips.map((s, i) =>
				i === index ? { ...s, link: text } : s,
			);
	...
	async function moveSpellUp(levelKey: string, index: number) {
		...
		if (levelKey === "cantrips") {
			...
			[updatedLearned[index - 1], updatedLearned[index]] = [
				updatedLearned[index],
				updatedLearned[index - 1],
			];
			char_spells = {
				...char_spells,
				[levelKey]: { ...level, learned: updatedLearned },
	...
	async function removeSpell(levelKey: string, index: number) {
		if (levelKey === "cantrips") {
			char_spells = {
				...char_spells,
				cantrips: char_spells.cantrips.filter((_, i) => i !== index),
			};
	...
	async function addCantrip() {
		...
		char_spells = {
			...char_spells,
			cantrips: [...char_spells.cantrips, spell],
		};
		newCantripName = "";
	...
	async function addSpell(levelKey: string) {
		...
		char_spells = {
			...char_spells,
			[levelKey]: { ...level, learned: [...level.learned, spell] },
		};
		newSpellNames = { ...newSpellNames, [levelKey]: "" };

		await app.fileManager.processFrontMatter(file, (fm) => {
			if (!fm.spells) fm.spells = {};
			if (!fm.spells[levelKey])
				fm.spells[levelKey] = {
					total_slots: 0,
					slots_expended: 0,
					learned: [],
	...
	function getSpellsaveDC(proficiencyBonus: number, scAbility: string) {
		let spellSaveDC =
			8 +
			proficiencyBonus +
			getAbilityModifier(char_abilities[scAbility]);
		return spellSaveDC;
	}

	function getSpellAttackBonus(proficiencyBonus: number, scAbility: string) {
		let spellAttackBonus =
			proficiencyBonus + getAbilityModifier(char_abilities[scAbility]);
...
<div class="sheet-container">
	...
	<div class="stat-grid">
		...
		<div class="stat-row">
			...
			<span class="mod"
				>{formatModifier(
					getAbilityModifier(char_abilities["dex"]),
				)}</span
			>
		</div>
	</div>
	...
	<div class="hitdice-block">
		...
		<div class="hitdice-row">
			<label class="stat-row">
				...
				<input
					...
					on:change={(e) =>
						updateHitDice("total", Number(e.currentTarget.value))}
				/>
			</label>
			...
			<label class="stat-row">
				...
				<input
					...
					on:change={(e) =>
						updateHitDice("used", Number(e.currentTarget.value))}
				/>
			</label>
			...
			<label class="stat-row">
				...
				<input
					...
					on:change={(e) =>
						updateHitDice("die", e.currentTarget.value)}
				/>
			</label>
		</div>
		...
		<div class="hitdice-remaining">
			Remaining: <span class="mod"
				>{char_hit_dice.total -
					char_hit_dice.used}{char_hit_dice.die}</span
			>
		</div>
	</div>
	...
	<div class="hp-block">
		...
		<label>
			...
			<input
				...
				value={char_hp.temp}
				on:change={(e) =>
					updateHP("temp", Number(e.currentTarget.value))}
			/>
			<span style="font-weight: bold">= </span>
			...
			<input
				...
				value={char_hp.max}
				on:change={(e) =>
					updateHP("max", Number(e.currentTarget.value))}
			/>
		</label>
	</div>
		...
		{#each abilityKeys as key}
			<div class="ability-box">
				...
				<input
					...
					on:change={(e) =>
						updateAbility(key, Number(e.currentTarget.value))}
				/>
				<span class="mod"
					>{formatModifier(
						getAbilityModifier(Number(char_abilities[key])),
					)}</span
				>
			</div>
		{/each}
		...
		{#each skillKeys as { skill, ability }}
			<div class="skill-box">
				<input
					...
					on:change={(e) =>
						updateSkill(skill, Boolean(e.currentTarget.checked))}
				/>
			...
			{#each char_proficiencies as proficiency, index}
				<li>
					<button
						class="icon-btn"
						disabled={index === 0}
						on:click={() => moveProficiencyUp(index)}>↑</button
					>
					<span class="item-name">{proficiency}</span>
					<button
						class="icon-btn"
						on:click={() => removeProficiency(index)}>✕</button
					>
				</li>
			{/each}
		</ul>
		...
		<div class="money-section">
			...
			<div class="currency-row">
				{#each currencyKeys as coin}
					<label class="stat-row">
						<span class="coin-label coin-{coin}"
		...
		<div class="attack-table">
			...
			{#each char_attacks as attack, index}
				<div class="attack-row">
					...
					/>
					<span class="mod"
						>{formatModifier(getAttackBonus(attack))}</span
					>
					<button
						class="icon-btn"
						on:click={() => removeAttack(index)}>✕</button
					>
				</div>
				...
				<div class="attack-properties-row">
					{#each WEAPON_PROPERTIES as prop}
						<label
							class="property-tag"
							class:active={attack.properties?.includes(prop) ??
								false}
						>
							<input
								type="checkbox"
								checked={attack.properties?.includes(prop) ??
									false}
								on:change={() =>
									toggleAttackProperty(index, prop)}
							/>
							{prop}
						</label>
					{/each}
			...
			<div class="attack-row attack-new">
				...
				<input
					...
					bind:checked={newAttack.proficient}
				/>
				<input
					class="text-input small"
					type="text"
			...
			<div class="attack-properties-row">
				{#each WEAPON_PROPERTIES as prop}
					<label
						class="property-tag"
						class:active={newAttackProperties.includes(prop)}
					>
						<input
							type="checkbox"
			...
			{#each char_traits as trait, index}
				<li>
					<button
						class="icon-btn"
						disabled={index === 0}
						on:click={() => moveTraitUp(index)}>↑</button
					>
					...
					<input
						...
						value={trait}
						on:change={(e) =>
							updateTrait(index, String(e.currentTarget.value))}
					/>
					<button class="icon-btn" on:click={() => removeTrait(index)}
						>✕</button
					>
				</li>
			{/each}
				...
				<li class="spell-row">
					...
					<div class="spell-actions">
						<button
							class="icon-btn"
							title="Paste link"
							on:click={() => pasteSpellLink("cantrips", index)}
							>📋</button
						>
						<button
							class="icon-btn"
							title="Copy link"
							on:click={() => copySpellLink("cantrips", index)}
							>🔗</button
						>
						<button
							class="icon-btn"
							...
							on:click={() =>
								openSpellInBrowser("cantrips", index)}
							>🌐</button
						>
						<button
							class="icon-btn"
							title="Move up"
							disabled={index === 0}
							on:click={() => moveSpellUp("cantrips", index)}
							>↑</button
						>
						<button
							class="icon-btn"
							title="Remove"
							on:click={() => removeSpell("cantrips", index)}
							>✕</button
						>
					</div>
				</li>
				...
				{#each level.learned as spell, index}
					<li class="spell-row">
						<input
							...
							on:change={() =>
								toggleSpellPrepared(levelKey, index)}
						/>
						...
						<div class="spell-actions">
							<button
								class="icon-btn"
								title="Paste link"
								on:click={() => pasteSpellLink(levelKey, index)}
								>📋</button
							>
							<button
								class="icon-btn"
								title="Copy link"
								on:click={() => copySpellLink(levelKey, index)}
								>🔗</button
							>
							<button
								class="icon-btn"
								title="Open in browser"
								on:click={() =>
									openSpellInBrowser(levelKey, index)}
								>🌐</button
							>
							<button
								class="icon-btn"
								disabled={index === 0}
								title="Move up"
								on:click={() => moveSpellUp(levelKey, index)}
								>↑</button
							>
							<button
								class="icon-btn"
								title="Remove"
								on:click={() => removeSpell(levelKey, index)}
								>✕</button
							>
						</div>
					</li>
